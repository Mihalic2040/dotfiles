...
func (Mgr *DbManager) DatabaseUpdateEventServer(m manifest.Manifest) {
	...
	defer func() {
		database.EventBus.Unsubscribe(db.DbUpdateEvent, eventListener)
		close(eventListener) // Close the channel when the goroutine exits
	}()

	for {
		select {
		case <-Mgr.stopCh:
			log.Println("DatabaseUpdateEventServer goroutine exiting.")
			return
		default:
			for range eventListener {
				runtime.EventsEmit(Mgr.wailsctx, "update")
...
func (Mgr *DbManager) ChangeListeningDb(m manifest.Manifest) {
	...
	select {
	case <-Mgr.stopCh:
		// Mgr.stopCh is already closed, do nothing
	default:
		// Mgr.stopCh is not closed, close it
		close(Mgr.stopCh)
